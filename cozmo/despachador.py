...
async def desk_security_guard(robot):
    ...
    while True:
        ...
        # Chequeo si hay despachos pendientes
        ...
        if jData['resp'] == "OK":
            print("****OK:")
            # Aca vamos a hacer que apile los cubos. Se copio el codigo 05_cube_stack.py
            await robot.say_text("Me acaba de llegar un pedido de Oracle").wait_for_completed()
        ...
        url = "https://howtoservice-gse00014621.uscom-east-1.oraclecloud.com/pedidos/cancelar"
        myResponse = requests.get(url, verify=False)
        jData = json.loads(myResponse.content)
        if jData['resp'] == "OK":
            print("****OK:")
            ##await robot.say_text("Que macana").wait_for_completed()
            await robot.play_anim_trigger(cozmo.anim.Triggers.MajorFail).wait_for_completed()
